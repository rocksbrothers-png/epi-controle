...
def build_bootstrap(connection, actor):
    ...
    return {
        ...
        'purchase_functions': fetch_purchase_function_links(connection, None if actor['role'] == 'master_admin' else actor['company_id']),
        'units': units,
        'employees': employees,
...
def get_actor_purchase_unit_scope(connection, actor):
    ...
    linked_employee_id = actor.get('linked_employee_id')
    if linked_employee_id and actor_role in ('buyer', 'approver'):
...
def fetch_purchase_function_links(connection, company_id=None):
    if connection is None:
        return []
    clauses, params = [], []
    if company_id not in (None, '', 'null'):
        clauses.append('prul.company_id = ?')
        params.append(int(company_id))
    where_sql = f"WHERE {' AND '.join(clauses)} " if clauses else ''
    rows = connection.execute(
        ...
        'JOIN companies ON companies.id = prul.company_id '
        'JOIN units ON units.id = prul.unit_id '
        f'{where_sql}'
        'ORDER BY companies.name, employees.name, prul.role_type, units.name',
        tuple(params)
    ).fetchall()
    items = []
    for row in rows:
...
def fetch_purchase_demands(connection, company_id, scope_unit_id=None):
    ...
    req_clauses = [
        'r.company_id = ?',
        "r.status IN ('solicitado', 'em análise', 'aprovado')",
        'NOT EXISTS (SELECT 1 FROM purchase_request_items pri WHERE pri.epi_request_id = r.id)'
    ]
    ...
    req_rows = connection.execute(
        f'SELECT r.id, r.company_id, r.unit_id, r.employee_id, r.epi_id, r.quantity, r.glove_size, r.size, r.uniform_size, r.status, r.requested_at, '
