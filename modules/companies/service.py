...
def validate_company_payload(connection, payload, company_id=None, *, get_commercial_settings, validate_cnpj, ensure_unique_company_cnpj, validate_logo_payload, normalize_plan_key, count_company_users):
...
def evaluate_company_block_status(connection, company_id, persist_expiration=True, *, get_company_by_id, count_company_users):
    ...
    if license_status == 'suspended': reasons.append('license_suspended')
    if license_status == 'expired': reasons.append('license_expired_by_contract')
    ...
    return {'company_id': int(company_id), 'blocked': bool(dedup), 'reasons': dedup, 'license_status': license_status, 'active_users': active_users, 'user_limit': user_limit, 'addendum_enabled': addendum_enabled, 'contract_end': contract_end}


def enforce_company_block_rules(connection, company_id, *, evaluate_company_block_status):
